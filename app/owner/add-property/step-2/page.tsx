...
//     try {
//       // Accept all common Google Maps URL formats
...
//     // Replace /place/ with /embed and add search query
//     if (url.includes('/maps/place/')) {
//       return url.replace('/maps/place/', '/maps/embed?pb=');
//     }
    
//     // Replace /maps/@coordinates with embed view
//     if (url.includes('/maps/@')) {
//       return url.replace('/maps/@', '/maps/embed?pb=');
//     }
    
//     // Fallback: convert /maps/ to /maps/embed/
...
//     const trimmedPropertyName = formData.propertyName.trim();
...
//     } else if (trimmedSubtitle.length > 150) {
//       newErrors.subtitle = "Maximum 150 characters";
//     }
    
//     if (!trimmedPropertyName) newErrors.propertyName = "Property name is required";
...
//                     // Immediate validation with broader Google Maps URL formats
//                     const trimmedValue = value.trim();
//                     const isGoogleMaps = 
//                       trimmedValue.includes('google.com/maps') ||
//                       trimmedValue.includes('maps.google.com') ||
//                       trimmedValue.includes('goo.gl/maps') ||
//                       trimmedValue.includes('maps.app.goo.gl');
                    
//                     if (!trimmedValue) {
//                       setErrors({ ...errors, googleMapLink: "Google Map link is required" });
//                     } else if (!isGoogleMaps) {
//                       setErrors({ ...errors, googleMapLink: "Please enter a valid Google Maps link" });
//                     } else {
...
export default function Step2Page() {
 ...
 const convertToEmbedURL = (url: string): string => {
  ...
  return `https://www.google.com/maps/embed/v1/place?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY}&q=${placeName}`;
    ...
    // Case 3: Place URL with place name only
    if (url.includes("/place/")) {
      const placeName = url.split("/place/")[1].split("/")[0];
      return `https://www.google.com/maps/embed/v1/place?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY}&q=${placeName}`;
    }
